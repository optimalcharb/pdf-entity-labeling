// this file has sections: Interfaces, Size functions, Position functions, Rect functions, Matrix functions
// ***INTERFACES***
...
function toIntPos(p: Position): Position {
  return { x: Math.floor(p.x), y: Math.floor(p.y) }
}
/** Clamp a Size so it never truncates right / bottom (ceil) */
function toIntSize(s: Size): Size {
...
export function boundingRect(rects: Rect[]): Rect | null {
  ...
  let minX = firstRect.origin.x,
    minY = firstRect.origin.y,
    maxX = firstRect.origin.x + firstRect.size.width,
    maxY = firstRect.origin.y + firstRect.size.height
