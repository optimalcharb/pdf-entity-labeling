...
export function createEmitter<T = any>(): Emitter<T> {
  ...
  const on = (l: Listener<T>): Unsubscribe => {
    listeners.add(l)
  ...
  return {
    ...
    on,
    off: (l: Listener<T>) => listeners.delete(l),
...
export function createBehaviorEmitter<T = any>(
  ...
  const baseOn = (listener: Listener<T>, options?: EventControlOptions): Unsubscribe => {
    let realListener: Listener<T> = listener
    let destroy = () => {}
    if (options) {
